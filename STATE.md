...
- KONDISI TERAKHIR (2026-09-19): Decode APK (run `35436973720`) SELESAI, artifact `/usr/tmp/opencode/decode`. Temuan besar: APK = single-Activity programmatic (5040 baris smali, NOL Fragment), `BubbleService` (bukan Floating), TANPA `ProfileMonitorService`, TANPA `POST_NOTIFICATIONS`, prefs pakai `app_bg_uri` (bukan `bg_image_path`). Game list di APK SUDAH BENAR (lewat `game_manager.sh` + JSON). Apply display SUDAH include density. Reset display SUDAH reset keduanya. Long-press 800ms + persist `hidden` SUDAH ADA (tanpa toast/vibrator). BatteryLab intent tanpa guard. Root tanpa timeout. Jalur: (a) overlay res, (b) patch smali (baca smali dulu), (c2) source Kotlin untuk fitur besar. PLAN.md diupdate. Sebelumnya: TAHAP 1+2, R1/F2 analisis, zip final 5.4M menunggu flash. Belum eksekusi kode. Belum merge.
